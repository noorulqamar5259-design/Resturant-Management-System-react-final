    ...
    <div className="min-h-screen bg-background flex flex-col transition-colors duration-500">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full py-16 px-4">
        <div className="text-center mb-16">
          ...
          <h1 className="text-5xl font-serif font-bold text-primary mt-2">Shopping Cart</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            {items.length > 0 ? (
              items.map(item => (
                <div key={item.id} className="bg-background rounded-[2rem] p-6 shadow-xl border border-border-dim flex flex-col md:flex-row items-center gap-8 group">
                  ...
                  <div className="flex items-center gap-4 bg-surface p-2 rounded-xl">
              ...
              <div className="text-center py-20 bg-background rounded-[3rem] border border-dashed border-border-dim">
                <span className="text-6xl block mb-6">🛒</span>
                <h3 className="text-2xl font-bold text-primary">Your cart is empty</h3>
                <Link to="/menu" className="inline-block mt-6 px-8 py-3 bg-accent text-white rounded-full font-bold hover:bg-accent/90 transition-all shadow-lg shadow-amber-900/20">Explore Menu</Link>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-background rounded-[2.5rem] p-10 shadow-2xl border border-border-dim sticky top-24">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="h-px bg-border-dim my-4"></div>
                <div className="flex justify-between text-xl font-bold text-primary">
                  <span>Total</span>
                  <span className="text-accent">${total.toFixed(2)}</span>
                </div>
              </div>
              <button className="w-full py-5 bg-accent hover:bg-accent/90 text-white rounded-2xl font-bold text-lg shadow-xl shadow-amber-900/20 transition-all transform active:scale-95 mb-4">
                Checkout Now
              </button>
              <p className="text-center text-xs text-secondary uppercase tracking-widest font-bold">Secure Payment</p>
            </div>
          </div>
        </div>
      </main>
